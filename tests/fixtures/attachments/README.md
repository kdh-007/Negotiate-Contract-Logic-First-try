...
걸 뒤늦게 발견해서(`nego/fields.py`의 `PERSONAL_FIELDS`가 API 응답에 적용하는
것과 같은 원칙) HWPX는 `Contents/section*.xml`의 해당 텍스트 노드를, PDF는
페이지 콘텐츠 스트림을 직접 편집해 지운 뒤 다시 커밋했다. 이름 자체는 오탐
위험 때문에 남겨뒀다 — `nego/attachments.py`의 `redact_personal_contacts()`
문서화 참고.
