...
def _merge_wrapped_parens(text: str) -> str:
    ...
    실측(2026-09-22, 리포트 화면에서 "7215409901"만 뜨고 이름이 안 붙는 걸
    사용자가 발견 — 두바이 의료기기전시회 한국관 공고문, `_CODE_REQUIREMENT_RE`
    docstring에 이미 적혀 있던 그 사례): 위와 달리 괄호 "안"이 아니라 이름표가
    끝나는 지점과 여는 괄호 "사이"에서 줄이 끊기는 경우도 있다 — "전시부스
    설치서비스\\n(7215409901)"처럼. `_extract_code_requirements`는 줄 단위로
    이름표를 찾기 때문에, 이러면 이름표가 있는 줄과 코드가 있는 줄이 갈라져
    이름을 통째로 잃고 코드만 남는다. 여는 괄호/대괄호 바로 앞의 줄바꿈(및
    앞뒤 공백)도 같은 이유로 제거해 한 줄로 합친다."""
    text = re.sub(r"[ \t]*\r?\n[ \t]*(?=[(\[])", "", text)
    out = []
    depth = 0
    for ch in text:
