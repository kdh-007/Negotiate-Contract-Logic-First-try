...
전수 확인, 사용자 지적으로 3차 보정): 최상위 절 번호매김은 4가지 관행이 있다 —
로마숫자를 같은 줄에 쓰는 "Ⅰ. 제목"(가장 흔함), 로마숫자와 제목을 **다른 줄에**
쓰는 "Ⅰ\\n제목"(박스형 제목틀 — 사용자 스크린샷으로 확인, 46/107건에서 발견),
"제N장", 아라비아숫자에 가나다 하위항목이 붙는 형태. 구조 인식률 96%(103/107).
나머지 4건은 표/서식 문서이거나(챕터 자체가 없음) 원문자(①②③) 등 아직 다루지
않는 번호매김이라 신뢰할 구조가 없어 통째로 반환한다(fail-open).
...
def split_sections(text: str) -> list[Section]:
    ...
    로마숫자(같은 줄/박스형 제목틀 모두) → 제N장 → 아라비아숫자(가나다 하위항목으로
    검증됨) 순으로 시도하고, 셋 다 신뢰할 만한 구조를 못 찾으면 전체 텍스트를 절
    1개(heading="")로 반환한다(fail-open) — 어설프게 쪼개서 조항 문장을 "절"로
    오인하는 것보다, 통째로 넘겨서 호출측이 원문 그대로 보는 편이 낫다.
    """
